...
# TO DO: Create a vector of 5 random values (between 1 and 10) and save it to a variable called "numbers".
...
all_numbers <- c(numbers, seq_numbers)
...
fuel_guzzlers <- mtcars[mtcars$mpg<20,]
...
mtcars_mini <- mtcars[c(1,3,5),names(mtcars) %in% c('hp','disp')]
mtcars_mini <- mtcars[c(1,3,5),names(mtcars) %in% c('disp','hp')]
